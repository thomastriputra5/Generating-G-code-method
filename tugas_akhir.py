...
class TugasAkhir(inkex.Effect):
    ...
    def effect(self):
        inkex.errormsg("Hello World")
        inkex.errormsg(self.options.laser_command)
        inkex.errormsg(self.options.laser_off_command)

        if self.options.raster_method == 1:  # R2L - Raster 2 Laser
            from raster2laser_gcode import GcodeExport
            inkex.errormsg("r2l imported")

            e = GcodeExport()
            inkex.errormsg("r2l instantiated")
            e.affect()
            inkex.errormsg("r2l affected")
            exit()
